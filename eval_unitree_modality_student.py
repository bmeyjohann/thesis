"""Evaluate a no-memory modality student on one physical locomotion cell."""
...
from unitree_multimodal_locomotion import MODALITIES, MultimodalNoMemoryActor, modality_mask, preprocess_student_input
...
def main() -> int:
    ...
    cfg.seed = args.seed
    camera_cfg = saved.get("camera", {"width": 80, "height": 60, "max_depth": 5.0})
    ...
    for _ in range(args.steps):
        ...
        with torch.inference_mode():
            action = student(proprio, {args.modality: terrain_obs}, availability)
    ...
    summary = {
        "modality": args.modality, "geometry": args.geometry, "material": args.material,
    ...
    print("[modality-eval] " + json.dumps(summary, sort_keys=True))
    env.close()
    return 0
