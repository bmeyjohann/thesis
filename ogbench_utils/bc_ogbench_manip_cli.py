"""Manipulation-specific behavior cloning CLI parsing."""
...
def build_bc_manip_parser() -> argparse.ArgumentParser:
    parser = build_train_parser()
    _attach_eval_args(parser)
    parser.add_argument(
        "--bc_collect_online_teacher_data",
        action="store_true",
        default=False,
        help=(
            "Collect additional expert labels from online interventions during training. "
            "Disabled by default so the BC baseline stays a pure demo-only imitation learner."
        ),
    )
    parser.set_defaults(**_BC_DEFAULTS)
    return parser


def parse_bc_manip_args():
    args = build_bc_manip_parser().parse_args()
    if getattr(args, "obs_mode", "state") != "state":
        raise ValueError("Behavior cloning manipulation entrypoint only supports obs_mode='state'.")
    env_name = str(getattr(args, "env_name", "") or "").lower()
    forbidden = ("maze", "pointmaze", "antmaze")
    if any(tok in env_name for tok in forbidden):
        raise ValueError(f"Behavior cloning manipulation entrypoint received maze env_name={args.env_name!r}.")
