...
- `scripts/vr_receiver_monitor.py`: Standalone receiver/monitor for raw VR controller streams; it can either listen for a pushed stream or connect to the persistent desktop publisher using an explicit or cached endpoint, and it can show an optional pygame status window with incoming controller/button state.
...
- `ogbench_utils/vr_teleop.py`: dependency-light raw VR transport and receiver utilities: local-IP discovery, JSON/TCP listen/connect/publish modes, endpoint cache helpers (`codex/vr_last_endpoint.json`), sender target prompts (pygame or console), optional status panel, raw controller helpers, and a manipulation action mapper/teleop interface that converts streamed controller pose/trigger state into 5D manip actions on the receiving machine.
