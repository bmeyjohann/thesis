...
- `scripts/vr_sender.py`: Desktop-side raw VR publisher for Windows/Linux. Default mode is a persistent server (`--mode serve`) that samples demo/OpenVR controller state continuously and lets laptop eval/train processes connect to it; legacy push mode is still available. It can remember the last endpoint in `codex/vr_last_endpoint.json`.
- `scripts/vr_receiver_monitor.py`: Standalone receiver/monitor for raw VR controller streams; it can either listen for a pushed stream or connect to the persistent desktop publisher using an explicit or cached endpoint, and it can show an optional pygame status window with incoming controller/button state plus live movement visualization (XY trail, yaw, trigger, trackpad, joystick) for both controllers.
...
- `ogbench/ogbench/manipspace/envs/manipspace_env.py`: Base 5D relative end-effector manipulation env; zero/no-op actions now hold the previously commanded joint/gripper targets instead of re-solving IK every step, and `initialize_arm()` seeds the PD actuator targets from the sampled reset pose so idle teleop does not churn nullspace targets.
...
- `ogbench_utils/vr_teleop.py`: dependency-light raw VR transport and receiver utilities: local-IP discovery, JSON/TCP listen/connect/publish modes, endpoint cache helpers (`codex/vr_last_endpoint.json`), sender target prompts (pygame or console), an optional status panel with controller motion/axis visualization, raw controller helpers, and a manipulation action mapper/teleop interface that converts streamed controller pose/trigger state into 5D manip actions on the receiving machine.
