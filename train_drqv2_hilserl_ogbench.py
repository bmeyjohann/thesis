#!/usr/bin/env python3
"""
DrQ-v2 + HIL-SERL-style human-in-the-loop baseline for OGBench.

This script mirrors the DrQ-v2 training loop but adds:
- A demo buffer (prefilled with teacher demos).
- Equal sampling from demo and RL buffers (50/50) when enabled.
- Intervention transitions stored in both buffers (demo + RL).
"""
...
def concat_replay_batches(batch_a: tuple, batch_b: tuple) -> tuple:
    if len(batch_a) != len(batch_b):
        raise ValueError("Replay batch mismatch when combining replay batches.")
    combined = []
...
def prefill_replay_with_demos(
    *,
    args,
    agent: DrQV2Agent,
    device: torch.device,
    target_storage: ReplayBufferStorage,
    target_label: str,
    record_progress,
    action_dim: int,
    ...
    target_episodes = int(args.demo_prefill_episodes)
    target_steps = int(args.demo_prefill_steps)
    if target_episodes <= 0 and target_steps <= 0:
        return
    ...
    demo_wrappers = [build_ogbench_wrapper(
        ...
        include_direction=demo_args.include_direction,
        include_velocity=demo_args.include_velocity,
        reward_type=demo_args.reward_type,
        dense_reward_scale=demo_args.dense_reward_scale,
        step_penalty=demo_args.step_penalty,
        reward_switch_after_steps=demo_args.reward_switch_after_steps,
        intervention_mode=demo_args.intervention_mode if demo_args.use_intervention else "none",
        teacher_type=demo_args.teacher_type,
        tolerance_type=demo_args.tolerance_type,
        tolerance_value=demo_args.tolerance_value,
        hard_block_lethal=demo_args.hard_block_lethal,
        intervention_enable_after_steps=demo_args.intervention_enable_after_steps,
        intervention_agent_mode="divergence",
        intervention_safety_margin_frac=demo_args.intervention_safety_margin_frac,
        intervention_release_steps=demo_args.intervention_release_steps,
        intervention_episode_prob=demo_args.intervention_episode_prob,
        intervention_episode_prob_min=demo_args.intervention_episode_prob_min,
    ...
    record_progress(f"[Demo] Prefilling {target_label} buffer")
    ...
    target_storage.add(time_step)

    try:
        while True:
            if target_episodes > 0 and total_episodes >= target_episodes:
                break
            if target_episodes <= 0 and target_steps > 0 and total_steps >= target_steps:
                break
            if time_step.last():
                total_episodes += 1
                action_history = init_action_history(action_dim, action_history_len)
...
def train():
    ...
    replay_dir_rl = buffer_root / "buffer_rl"
    replay_storage_rl = ReplayBufferStorage(data_specs, replay_dir_rl)
    ...
    rl_batch = max(1, args.batch_size - demo_batch)
    if demo_batch <= 0:
        demo_batch = 0
        rl_batch = args.batch_size

    rl_loader_full = make_replay_loader(
        replay_dir_rl,
    ...
    rl_loader_part = make_replay_loader(
        replay_dir_rl,
        max_size=args.replay_buffer_size,
        batch_size=rl_batch,
        num_workers=args.replay_buffer_num_workers,
    ...
    if demo_storage is not None and demo_batch > 0:
        demo_loader_part = make_replay_loader(
            demo_dir,
            max_size=args.demo_buffer_capacity,
    ...
    replay_storage_rl.add(time_step)
    ...
    while total_env_steps < args.total_timesteps:
        ...
        replay_storage_rl.add(next_time_step)
        if teacher_intervened and demo_storage is not None:
            demo_storage.add(next_time_step)
        ...
        if total_env_steps >= args.num_expl_steps:
            if replay_storage_rl.num_episodes() == 0:
                continue
            use_demo = demo_storage is not None and demo_batch > 0 and demo_storage.num_episodes() > 0
            if use_demo:
                if rl_iter_part is None:
                    rl_iter_part = iter(rl_loader_part)
                if demo_iter_part is None and demo_loader_part is not None:
                    demo_iter_part = iter(demo_loader_part)
                try:
                    batch_rl = next(rl_iter_part)
                ...
                try:
                    ...
                    demo_iter_part = iter(demo_loader_part)
                    batch_demo = next(demo_iter_part)
                combined_batch = concat_replay_batches(batch_rl, batch_demo)
                replay_iter = iter([combined_batch])
            else:
                if rl_iter_full is None:
                    rl_iter_full = iter(rl_loader_full)
                try:
                    batch_rl = next(rl_iter_full)
                except StopIteration:
                    rl_iter_full = iter(rl_loader_full)
                    batch_rl = next(rl_iter_full)
                replay_iter = iter([batch_rl])
