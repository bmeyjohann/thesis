...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-safetycar_goal2_pretrain_dense_plus_sparse_${TS}}"
LOG_INTERVAL="${LOG_INTERVAL:-2000}"
EVAL_INTERVAL="${EVAL_INTERVAL:-10000}"
NUM_EVAL_EPISODES="${NUM_EVAL_EPISODES:-10}"
SAVE_INTERVAL="${SAVE_INTERVAL:-10000}"

echo "Starting SafetyCar pretraining run."
echo "Reward mode: dense_plus_sparse"
echo "This launcher is intentionally long-running; stop it manually once success is meaningful."
echo "Current SafetyGym own-method path is single-env only, so this pretrain launcher uses num_envs=1."
