...
NET_TAG="${NET_TAG:-big}"
TAG="${TAG:-firstgoal_clearance4_margin015_cost002_ramp5k}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-20000}"
CHECKPOINT_INTERVAL="${CHECKPOINT_INTERVAL:-5000}"

EXP_PREFIX="${EXP_PREFIX:-safetycar_min_stage2_goalbase_clearance_${NET_TAG}_${TAG}}"
...
/home/benjamin/thesis/scripts/run_safetycar_minimal_human_ready_local.sh \
  ...
  COST_PENALTY="${COST_PENALTY:--0.02}" \
  COST_PENALTY_WARMUP_STEPS="${COST_PENALTY_WARMUP_STEPS:-0}" \
  COST_PENALTY_RAMP_STEPS="${COST_PENALTY_RAMP_STEPS:-5000}" \
  CLEARANCE_PENALTY_SCALE="${CLEARANCE_PENALTY_SCALE:-4.0}" \
