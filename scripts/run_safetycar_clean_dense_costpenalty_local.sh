...
BASE_TAG="${BASE_TAG:-safetycar_clean_dense_costpenalty}"
TS="${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
RUN_NAME="${RUN_NAME:-${BASE_TAG}_${TS}}"
WANDB_GROUP="${WANDB_GROUP:-}"

echo "Starting SafetyCar clean dense run."
echo "run_name=${RUN_NAME}"
echo "timesteps=${TOTAL_TIMESTEPS}"
echo "step_penalty=${STEP_PENALTY}"
echo "cost_penalty=${COST_PENALTY} (set nonzero only if you explicitly want extra collision shaping)"
