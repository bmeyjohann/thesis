...
CONDA_BIN="${CONDA_BIN:-/home/benjamin/miniconda3/bin/conda}"
...
BASE_TAG="${BASE_TAG:-safetycar_clean_dense_native_costpenalty}"
TS="${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
WANDB_GROUP="${WANDB_GROUP:-${BASE_TAG}_${TS}}"

run_case() {
  local reward_mode="$1"
  local name="$2"

  "${CONDA_BIN}" run -n "${CONDA_ENV}" "${PYTHON_BIN}" /home/benjamin/thesis/train_fast_sac_safetygym.py \
    --env_name "${ENV_NAME}" \
    --exp_name "${name}" \
...
DENSE_NAME="${DENSE_NAME:-${BASE_TAG}_dense_${TS}}"
NATIVE_NAME="${NATIVE_NAME:-${BASE_TAG}_native_${TS}}"

echo "Starting sequential SafetyCar clean reward comparison."
echo "Dense run:  ${DENSE_NAME}"
echo "Native run: ${NATIVE_NAME}"
echo "W&B group:  ${WANDB_GROUP}"
echo "Timesteps:  ${TOTAL_TIMESTEPS}"
echo "Eval/save interval: ${CHECKPOINT_INTERVAL}"
echo "Step penalty: ${STEP_PENALTY}"
echo "Cost penalty: ${COST_PENALTY} (default 0.0; collision is logged as metrics unless explicitly shaped)"

run_case "dense" "${DENSE_NAME}"
run_case "native" "${NATIVE_NAME}"
