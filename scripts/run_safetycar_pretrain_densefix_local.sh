...
EXPORT_FINAL_REPLAY_DATASET="${EXPORT_FINAL_REPLAY_DATASET:-0}"
EXPORT_DATASET_DIR="${EXPORT_DATASET_DIR:-}"

WANDB_PROJECT="${WANDB_PROJECT:-thesis-safetygym}"
EXPERIMENT_TAG="${EXPERIMENT_TAG:-safetycar_goal2_densefix}"
TS="$(date +%Y%m%d_%H%M%S)"
WANDB_GROUP="${WANDB_GROUP:-${EXPERIMENT_TAG}}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-${EXPERIMENT_TAG}_pretrain_${REWARD_MODE}_${TS}}"
...
echo "Starting SafetyCar dense-fix pretraining run."
echo "Reward mode: ${REWARD_MODE}"
echo "The dense reward wrapper now suppresses fake negative spikes on goal-resample steps."
echo "Collision/object-contact costs remain outside the reward."
...
if [[ "${EXPORT_FINAL_REPLAY_DATASET}" == "1" ]]; then
  cmd+=(--export_final_replay_dataset)
fi
if [[ -n "${EXPORT_DATASET_DIR}" ]]; then
  cmd+=(--export_dataset_dir "${EXPORT_DATASET_DIR}")
fi
