...
SAFE_BASE_CKPT="${SAFE_BASE_CKPT:-/home/benjamin/thesis/models/safetygym_minimal/safetycar_min_safe_dense_cost005_nostep_big_20260402_195329/step_5000.pt}"
TIMESTAMP="${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
NET_TAG="${NET_TAG:-big}"
CURRICULUM_TAG="${CURRICULUM_TAG:-cost001_fwd002_bwd001_head003}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-20000}"
CHECKPOINT_INTERVAL="${CHECKPOINT_INTERVAL:-5000}"

EXP_PREFIX="${EXP_PREFIX:-safetycar_min_stage2_safe_to_goal_${NET_TAG}_${CURRICULUM_TAG}}"
...
/home/benjamin/thesis/scripts/run_safetycar_minimal_human_ready_local.sh \
  ...
  COST_PENALTY_RAMP_STEPS="${COST_PENALTY_RAMP_STEPS:-15000}" \
  CLEARANCE_PENALTY_SCALE="${CLEARANCE_PENALTY_SCALE:-0.0}" \
  CLEARANCE_MARGIN="${CLEARANCE_MARGIN:-0.0}" \
  FORWARD_REWARD_SCALE="${FORWARD_REWARD_SCALE:-0.002}" \
  BACKWARD_PENALTY_SCALE="${BACKWARD_PENALTY_SCALE:-0.001}" \
  HEADING_REWARD_SCALE="${HEADING_REWARD_SCALE:-0.003}" \
