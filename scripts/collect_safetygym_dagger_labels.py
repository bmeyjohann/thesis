...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Collect Safety-Gym DAgger labels: execute student, label with scripted teacher.")
    p.add_argument("--model_path", type=str, required=True)
    p.add_argument("--student_policy", type=str, default="fastsac", choices=["fastsac", "imitation", "ppo"])
    p.add_argument("--student_intervention_threshold", type=float, default=0.0)
...
def _load_ckpt_defaults(args: argparse.Namespace) -> None:
    ...
    for name in (
        ...
        "clearance_margin",
        "clearance_penalty_mode",
        "clearance_penalty_temperature",
        "terminate_on_goal",
    ):
        if name in cfg:
...
def _make_collection_env(args: argparse.Namespace):
    env = make_safety_env(
        args.env_name,
...
def main() -> int:
    ...
    if str(args.student_policy).strip().lower() == "fastsac":
        _load_ckpt_defaults(args)
    device = torch.device("cuda" if args.device == "auto" and torch.cuda.is_available() else ("cpu" if args.device == "auto" else args.device))
    env = _make_collection_env(args)
    ...
    student_policy = str(args.student_policy).strip().lower()
    actor = None
    obs_preprocess = torch.nn.Identity()
    imitation_student = None
    ppo_student = None
    ppo_vecnormalize = None
    if student_policy == "fastsac":
        actor = SafetyActor(
            n_obs=obs_dim,
            n_act=act_dim,
            num_envs=1,
        ...
        if obs_norm_state and EmpiricalNormalization is not None:
            ...
            obs_preprocess.eval()
    elif student_policy == "imitation":
        imitation_student = LearnedInterventionPolicyController(
            checkpoint_path=args.model_path,
            action_low=action_low,
            action_high=action_high,
            intervention_threshold=float(args.student_intervention_threshold),
            device=str(device),
        )
    elif student_policy == "ppo":
        ...
        if vecnormalize_path is not None:
            ...
            expected_dim = int(np.prod(expected_shape)) if expected_shape else obs_dim
            ppo_env_args = copy.copy(args)
            if expected_dim != obs_dim:
                # The original PPO source policy used base lidar observations;
                # DAgger collectors may store richer observations for cloning.
                ppo_env_args.obs_mask_mode = "none"
            tmp_env = DummyVecEnv([lambda: _make_collection_env(ppo_env_args)])
            ...
            print(f"[DAgger] loaded PPO VecNormalize stats: {vecnormalize_path}", flush=True)
    else:
        raise ValueError(f"Unsupported student_policy: {args.student_policy}")

    teacher = build_human_controller(
    ...
    try:
        while episodes < int(args.num_episodes) and steps < int(args.max_steps):
            if student_policy == "fastsac":
                ...
                with torch.inference_mode():
                    ...
                    student_action = mean[0].detach().cpu().numpy().astype(np.float32)
                if bool(args.scale_actor_to_env_bounds):
                    student_action = scale_action_np(student_action, env.action_space)
            else:
                if student_policy == "imitation":
                    assert imitation_student is not None
                    student_action = imitation_student.get_action(obs=obs, env=env)
                    ...
                    if ppo_obs.shape[-1] != expected_dim:
                        if ppo_obs.shape[-1] > expected_dim:
                            ...
                            raise ValueError(
                                f"PPO student expects obs_dim={expected_dim}, got collector obs_dim={ppo_obs.shape[-1]}"
                            )
                    if ppo_vecnormalize is not None:
                        ppo_obs = ppo_vecnormalize.normalize_obs(ppo_obs)
                    student_action, _ = ppo_student.predict(ppo_obs, deterministic=True)
            ...
            teacher_action = clip_action_to_space(np.asarray(teacher_action, dtype=np.float32), env.action_space)

            next_obs, reward, cost, terminated, truncated, _info = env.step(student_action)
            next_obs = np.asarray(next_obs, dtype=np.float32).reshape(-1)
            obs_rows.append(obs.copy())
            action_rows.append(teacher_action.copy())
            ...
            student_rows.append(student_action.copy())
            ...
            if terminated or truncated:
                ...
                if imitation_student is not None:
                    imitation_student.reset()
        ...
        if imitation_student is not None:
            imitation_student.close()
        teacher.close()
        env.close()

    metadata = {
        "env_name": str(args.env_name),
        "source_model_path": str(Path(args.model_path).expanduser()),
        "label_teacher": "scripted_geo",
        "executed_policy": student_policy,
        "obs_mask_mode": str(args.obs_mask_mode),
        "car_action_mode": str(args.car_action_mode),
    ...
    print({"dataset_path": str(path), "episodes": int(episodes), "rows": int(len(obs_rows))}, flush=True)
    return 0
