...
RUN_NAME="${RUN_NAME:-scan_teacher_fixed_axes_wallfollow_$(date +%Y%m%d_%H%M%S)}"

python -u /home/benjamin/thesis/eval_unitree_nav_baselines.py \
  --controller scan_teacher \
  --device "${DEVICE:-cuda:0}" \
  --num-envs 1 \
  --num-episodes "${NUM_EPISODES:-8}" \
