...
for i in $(seq 1 720); do
  ...
  echo "[offline-bc] waiting for dataset: $DATASET_PATH ($i/720)"
  sleep 10
done
if [[ ! -s "$DATASET_PATH" ]]; then
...
exec /home/benjamin/thesis/scripts/run_safetycar_minimal_human_ready_local.sh \
  EXP_NAME=safetycar_offline_astar_bc_h256_60kdata_evalfix_20260514 \
  EXP_PREFIX=safetycar_offline_astar_bc_h256_60kdata_evalfix \
  VARIANT=own \
  ...
  WANDB_GROUP=safetycar_curriculum_astar_offlinebc_20260514 \
  SEED=270 \
