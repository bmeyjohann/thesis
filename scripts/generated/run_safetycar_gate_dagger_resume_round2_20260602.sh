...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib-thesis-gate-dagger-resume}"
...
SEED="${SEED:-7}"
BASE_SLICE="${BASE_SLICE:-$DATA_ROOT/base_5000.npz}"
DAGGER1="${DAGGER1:-$DATA_ROOT/dagger_round1_10000.npz}"
DAGGER2="${DAGGER2:-$DATA_ROOT/dagger_round2_5000.npz}"
BC_EPOCHS="${BC_EPOCHS:-25}"
DAGGER_STEPS="${DAGGER_STEPS:-5000}"
EVAL_EPISODES="${EVAL_EPISODES:-16}"
PLOT_EPISODES="${PLOT_EPISODES:-6}"
mkdir -p "$OUT_ROOT" "$DATA_ROOT" "$MODEL_ROOT" "$EVAL_ROOT"

if [[ ! -s "$BASE_SLICE" || ! -s "$DAGGER1" || ! -s "$MODEL_ROOT/round1_base5k_plus_dagger/best.pt" ]]; then
  echo "ERROR: missing prerequisite base/dagger1/round1 artifacts in $OUT_ROOT" >&2
  exit 2
fi
...
eval_bc() {
  ...
  mkdir -p "$out"
  echo "[eval] name=$name device=$device episodes=$EVAL_EPISODES"
...
variants = ["round0_base5k", "round1_base5k_plus_dagger", "round2_base5k_plus_dagger"]
...
md += ["## DAgger Train", "", "| variant | rows | int rows | non-int rows | val acc | val precision | val recall | val action MSE |",
       "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"]
for r in train_rows:
    md.append(f"| {r['variant']} | {int(float(r['rows']))} | {int(float(r['intervened_rows']))} | {int(float(r['non_intervened_rows']))} | {float(r['val_decision_accuracy']):.3f} | {float(r['val_decision_precision']):.3f} | {float(r['val_decision_recall']):.3f} | {float(r['val_action_mse_intervened']):.4f} |")
md += ["", "## DAgger Collection", "", "| round | rows | oracle int rate | learned int rate | gate disagree | action disagree |",
       "| ---: | ---: | ---: | ---: | ---: | ---: |"]
for r in collect_rows:
    md.append(f"| {r['round']} | {int(float(r['rows']))} | {float(r['oracle_intervention_rate']):.3f} | {float(r['learned_intervention_rate']):.3f} | {float(r['gate_disagreement_rate']):.3f} | {float(r['action_disagreement_rate_when_oracle_intervenes']):.3f} |")
md += ["", "## Closed-Loop Eval", "", "| variant | controller | episodes | cost eps | success | int frac |",
       "| --- | --- | ---: | ---: | ---: | ---: |"]
for r in eval_rows:
    md.append(f"| {r['variant']} | {r['controller']} | {r['episodes']} | {r['episodes_with_cost']} | {float(r['success_rate']):.3f} | {float(r['mean_intervention_fraction']):.3f} |")
(out_root / "summary.md").write_text("\n".join(md) + "\n", encoding="utf-8")
print(json.dumps({"summary_md": str(out_root / "summary.md"), "dagger_eval_summary": str(out_root / "dagger_eval_summary.csv")}, sort_keys=True), flush=True)
PY
