...
for i in $(seq 1 720); do
  ...
  echo "[offline-bc-goalblock] waiting for dataset: $DATASET_PATH ($i/720)"
  sleep 10
done
if [[ ! -s "$DATASET_PATH" ]]; then
...
exec /home/benjamin/thesis/scripts/run_safetycar_minimal_human_ready_local.sh \
  EXP_NAME=safetycar_offline_astar_bc_h256_b512_goalblockbc_60kdata_20260515 \
  EXP_PREFIX=safetycar_offline_astar_bc_h256_b512_goalblockbc_60kdata \
  VARIANT=own \
  ...
  ACTOR_BC_GOAL_BLOCK_WEIGHT_SCALE=10.0 \
  ACTOR_BC_GOAL_BLOCK_WEIGHT_MAX=10.0 \
