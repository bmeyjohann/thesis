...
# Task2 run: "your method" + demo buffer (sparse reward variant)
# - sparse reward mode
...
#
# This mirrors scripts/run_cube_triple_task2_dense_pref_demo20_deviation.sh
# except cube reward mode is sparse_intermediate.
#
# Optional overrides:
#   PROJECT=ogbench-manip-reward-debug GAMMA=0.97 TOTAL_TIMESTEPS=300000 \
#   bash scripts/run_cube_triple_task2_sparse_pref_demo20_deviation.sh
...
if [[ "${SAVE_INTERVAL}" -lt 10000 ]]; then
  echo "[run_cube_triple_task2_sparse_pref_demo20_deviation] SAVE_INTERVAL=${SAVE_INTERVAL} is too low; clamping to 10000."
  SAVE_INTERVAL=10000
fi
...
EXP_NAME="${EXP_NAME:-cube_triple_task2_sparse_pref_demo20_dev_${TS}}"
