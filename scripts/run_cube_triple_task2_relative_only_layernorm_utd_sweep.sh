...
# Two-run sweep cloned from the provided task2 command with:
...
if [[ "${SAVE_INTERVAL}" -lt 10000 ]]; then
  echo "[run_cube_triple_task2_relative_only_layernorm_utd_sweep] SAVE_INTERVAL=${SAVE_INTERVAL} is too low; clamping to 10000."
  SAVE_INTERVAL=10000
fi
...
EXP_UTD1="${EXP_UTD1:-cube_triple_task2_relonly_layernorm_utd1_smaller_actor_small_critic__no_stopgrad_num_critics_2_${TS}}"
EXP_UTD5="${EXP_UTD5:-cube_triple_task2_relonly_layernorm_utd5_smaller_actor_small_critic__no_stopgrad_num_critics_2_${TS}}"
