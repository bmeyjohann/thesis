...
EXPERT_CHECKPOINT_PATH="${EXPERT_CHECKPOINT_PATH:-${TEACHER_CHECKPOINT_PATH:-}}"
if [[ -z "$EXPERT_CHECKPOINT_PATH" ]]; then
  echo "Set EXPERT_CHECKPOINT_PATH=/path/to/imitation/best.pt" >&2
  exit 2
fi

TIMESTAMP="${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
EXP_PREFIX="${EXP_PREFIX:-safetycar_min_own_learned_teacher}"
EXP_NAME="${EXP_NAME:-${EXP_PREFIX}_${TIMESTAMP}}"

VARIANT=own \
USE_INTERVENTION=1 \
HUMAN_INPUT_DEVICE=learned \
...
EXP_NAME="$EXP_NAME" \
"$ROOT/scripts/run_safetycar_minimal_human_ready_local.sh"
