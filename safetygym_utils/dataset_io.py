...
DEFAULT_SAFETYGYM_DATASET_DIR = Path.home() / ".config" / "thesis" / "datasets" / "safetygym"
...
def build_dataset_path(
    *,
    env_name: str,
    dataset_dir: Path | str = DEFAULT_SAFETYGYM_DATASET_DIR,
    label: str = "human_gamepad_demo",
...
def save_transition_dataset(
    *,
    ...
    if costs is not None:
        payload["costs"] = np.asarray(costs, dtype=np.float32).reshape(-1)
...
def load_transition_dataset(path: Path | str) -> dict[str, Any]:
    ...
    with np.load(source, allow_pickle=True) as data:
        ...
        out = {
            ...
            "costs": np.asarray(data["costs"], dtype=np.float32).reshape(-1) if "costs" in data.files else None,
            "student_actions": (
                np.asarray(data["student_actions"], dtype=np.float32)
                if "student_actions" in data.files
                else np.asarray(data["actions"], dtype=np.float32)
            ),
            "teacher_intervened": (
...
def extend_buffer_from_dataset(
    *,
    ...
    if obs.ndim != 2 or next_obs.ndim != 2 or actions.ndim != 2:
        raise ValueError(f"Dataset {dataset_path} must contain 2D observation/action arrays.")
    ...
    if filter_mode == "intervened":
        selector = teacher_intervened.astype(bool)
    elif filter_mode == "non_intervened":
        selector = ~teacher_intervened.astype(bool)
    else:
        selector = np.ones((obs.shape[0],), dtype=np.bool_)

    indices = np.nonzero(selector)[0].tolist()
    if int(max_rows) > 0:
        ...
        td = TensorDict(
            {
                ...
                "teacher_intervened": torch.as_tensor(
                    teacher_intervened[idx : idx + 1], device=device, dtype=torch.bool
                ),
                "next": {
